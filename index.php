...
if($user){
	...
	if ($_GET['c'] != "1") {
     ...
     if($_POST['verifykey'] == $user['claim_cryptokey']){
      ...
      } elseif ($_SESSION['checker'] != $_POST['secc2']) {
          $content .= alert("danger", "Wrong Secondary Captcha. <a href='index.php'>Try again</a>.");
   				...
   				if(checkDirtyIp($realIpAddressUser, $iphubApiKey) == true && $VPNShield == "yes"){
   					...
   					if($IpCheck >= 1){
   						...
   						if($IpCheckBan >= 1 || $AddressCheckBan >= 1){
   							...
   							if ($referralPercent >= 1 && $user['referred_by'] != 0) {
			...
			if(!$addressCheck){
				...
				if($_COOKIE['refer']){
					if(is_numeric($_COOKIE['refer'])){
						...
						$referID = $AddressCheck == 1 ? $referID2 : 0;
					} else {
						$referID = 0;
					}
				} else {
